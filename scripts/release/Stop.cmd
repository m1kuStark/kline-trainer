...
exit /b 1
:stop
rem --stop targets only the service this launcher recorded (state + health
rem identity, exact verified PID). It never kills unknown processes.
"%NODE_EXE%" "%SCRIPT_DIR%launcher.cjs" --stop %*
set "EXIT_CODE=%ERRORLEVEL%"
if "%EXIT_CODE%"=="0" exit /b 0
echo.
echo [error] Stop failed. See the hints above; the launcher refuses to signal
echo a process it cannot verify as this trainer. Database and logs are kept
echo in the data directory (default: %USERPROFILE%\.a-share-kline-trainer).
echo.
pause
exit /b %EXIT_CODE%
